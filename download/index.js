...
if (params.has('plugin')) {
  ...
  if (plugins.indexOf(request.trim()) > -1) {
    startDownload(pluginLink.format({name: request}), `${request}.plugin.js`);
  ...
  if (themes.indexOf(request.trim()) > -1) {
    startDownload(themeLink.format({name: request}), `${request}.theme.css`);
